...
export function startTickers(root = document) {
  ...
  /* Checked before anything is cloned: a row that will never scroll should
     read as the one set of events it is, not as that set repeated. */
...
/**
 * Restore the layered orbit motion used by the reference infrastructure art.
 * The exported HTML only retained each ring's final rotation, so the CSS
 * animation is activated while the artwork is near the viewport.
 */
export function startInfrastructureOrbit(root = document) {
  const section = root.querySelector("#infrastructure");
  if (!section) return;
  ...
  if (!("IntersectionObserver" in window)) {
    section.classList.add("forg-orbit-active");
    return;
  }

  const observer = new IntersectionObserver(
    ([entry]) => section.classList.toggle("forg-orbit-active", entry.isIntersecting),
